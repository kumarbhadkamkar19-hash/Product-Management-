...
exports.getProduct = async (req, res) => {
  try {
    const product = await productService.getProduct(req.params.id, req.admin._id);
...
exports.updateProduct = async (req, res) => {
  try {
    const product = await productService.updateProduct(req.params.id, req.body, req.admin._id);
...
exports.deleteProduct = async (req, res) => {
  try {
    const result = await productService.deleteProduct(req.params.id, req.admin._id);
    res.json({ success: true, ...result });
  } catch (err) { handleError(res, err); }
};